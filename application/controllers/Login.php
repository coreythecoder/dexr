...
class Login extends CI_Controller 
{
	...
	{	
		$this->template->set_error_view("error/login_error.php");
		$this->template->set_layout("layout/login_layout.php");
		if ($this->user_model->check_block_ip()) {echo 1; exit();
			$this->template->error(lang("error_26"));
		}
		...
		setcookie($config . "un", $email, time()+$ttl, "/", "app.dexr.io");
		setcookie($config . "tkn", $token, time()+$ttl, "/", "app.dexr.io");
	...
	{	
		...
		$email_template->message);
                ...
                sendEmail($email, "forgotpassword@yoliya.com", $email_template->title, $email_template->message, $email_template->message);
